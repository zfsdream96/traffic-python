...
from(
select ceil((GETONLONGITUDE-103.6985766)/0.01193061)*100+ ceil((GETONLATITUDE -30.3159936)/0.0098227) as 上车点编号,
ceil((GETOFFLONGITUDE-103.6985766)/0.01193061)*100+ ceil((GETOFFLATITUDE -30.3159936)/0.0098227) as 下车点编号,count(*) as 计数,
...
and operationtime>0 and operationmil<65 and operationmil>1
)
group by (ceil((GETONLONGITUDE-103.6985766)/0.01193061)*100+ ceil((GETONLATITUDE -30.3159936)/0.0098227),ceil((GETOFFLONGITUDE-103.6985766)/0.01193061)*100+ ceil((GETOFFLATITUDE -30.3159936)/0.0098227)))
where std<5;
